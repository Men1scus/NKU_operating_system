...
buddy_system_alloc_pages(size_t n)
{
    ...
    uint32_t order = MAX_ORDER - 1;// 19->32  13 8 4 1
    while (n < (1 << order))
    {
        order -= 1;
    }
...
buddy_system_check(void)
{
    ...
    cprintf("Freeing 1 page...\n");
    buddy_system_free_pages(p0, 1);
    print_free_list_status(); // 释放后状态

    // 测试 2 页和 4 页的分配
    cprintf("Allocating 2 pages...\n");
    p1 = buddy_system_alloc_pages(2);
    assert(p1 != NULL);
    print_free_list_status(); // 分配2页后状态

    cprintf("Allocating 4 pages...\n");
    p2 = buddy_system_alloc_pages(4);
    assert(p2 != NULL);
    print_free_list_status(); // 分配4页后状态

    // 测试释放并合并 4 页
    cprintf("Freeing 4 pages...\n");
    buddy_system_free_pages(p2, 4);
    print_free_list_status(); // 释放4页后状态

    // 测试 8 页分配
    cprintf("Allocating 8 pages...\n");
    p3 = buddy_system_alloc_pages(8);
    assert(p3 != NULL);
    print_free_list_status(); // 分配8页后状态

    // 测试释放 8 页
    cprintf("Freeing 8 pages...\n");
    buddy_system_free_pages(p3, 8);
    print_free_list_status(); // 释放8页后状态

    // 释放 2 页并合并
    cprintf("Freeing 2 pages...\n");
    buddy_system_free_pages(p1, 2);
    print_free_list_status(); // 释放2页后状态
